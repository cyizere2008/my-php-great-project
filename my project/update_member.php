...
<?php
    ...
    $query = mysqli_query($conn, "SELECT * FROM members WHERE memberId= $update_id");
...
?>
    <form class="bg-white w-[90%] md:w-[60%] lg:w-[40%] gap-4 mt-16 p-8 flex flex-col rounded-lg mx-auto shadow-2xl" action="./update_members.php" method="POST">
        ...
        <div>
            ...
            <input id="description" class="border border-gray-300 text-gray-800 w-full p-2 rounded-md focus:ring-2 focus:ring-orange-500" type="text" name="address" value='<?php echo htmlspecialchars($record['address']); ?>'>
        </div>
